  ...
  <div class="page-card">
    ...
    <div class="split-grid">
      ...
      <section class="section-card">
        ...
        <table>
          <thead>
            <tr><th>委托号</th><th>客户号</th><th>状态</th><th>日期</th><th>操作</th></tr>
          </thead>
          <tbody>
            <tr v-for="item in rows" :key="item.orderId">
              ...
              <td>{{ item.status }}</td>
              <td>{{ item.date }}</td>
              <td><button type="button" @click="cancel(item)">撤单</button></td>
            </tr>
          </tbody>
        </table>
...
import { cancelOrderApi, mockSubmitWithdraw } from "../../api/trade";
...
const cancel = async (item) => {
  try {
    await cancelOrderApi(item.orderId, { customerCode: item.customerCode, withdrawReason: "用户申请" });
  } catch {
    await mockSubmitWithdraw({
